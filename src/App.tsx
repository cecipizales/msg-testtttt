...
const gltfUrls = ["/assets/1.glb", "/assets/2.glb", "/assets/3.glb"];
...
function Mosaic({
  ...
  const { geometry: geo1, material: origMat1 } = useMemo(
    () => extractAndNormalizeGeometry(gltf1),
    [gltf1],
  );
  const { geometry: geo2, material: origMat2 } = useMemo(
    () => extractAndNormalizeGeometry(gltf2),
    [gltf2],
  );
  const { geometry: geo3, material: origMat3 } = useMemo(
    () => extractAndNormalizeGeometry(gltf3),
    [gltf3],
  );
  ...
  useFrame(({ clock }) => {
      ...
      for (let x = 0; x < GRID_COLS; x++) {
        ...
        if (isInit) {
          ...
          if (diff > 120) {
            ...
            // Cap velocity
            const maxVel = 5.0;
            velocities[idx3 + 0] = Math.max(
              -maxVel,
              Math.min(maxVel, velocities[idx3 + 0]),
            );
            velocities[idx3 + 1] = Math.max(
              -maxVel,
              Math.min(maxVel, velocities[idx3 + 1]),
            );
            velocities[idx3 + 2] = Math.max(
              -maxVel,
              Math.min(maxVel, velocities[idx3 + 2]),
            );
          }
        }
        ...
        if (colorFilter === "grid") {
          ...
          if (colorFilter === "grayscale") {
            ...
            r = Math.min(
              255,
              Math.max(0, luminance + (r - luminance) * saturationBoost),
            );
            g = Math.min(
              255,
              Math.max(0, luminance + (g - luminance) * saturationBoost),
            );
            b = Math.min(
              255,
              Math.max(0, luminance + (b - luminance) * saturationBoost),
            );
          }
        ...
        const posX =
          -(x - GRID_COLS / 2) * spacing +
          spacing / 2 +
          displacements[idx3 + 0];
        const posY =
          -(y - GRID_ROWS / 2) * spacing +
          spacing / 2 +
          displacements[idx3 + 1];
    ...
    if (mesh1Ref.current) {
      ...
      if (mesh1Ref.current.instanceColor)
    ...
    if (mesh2Ref.current) {
      ...
      if (mesh2Ref.current.instanceColor)
        mesh2Ref.current.instanceColor.needsUpdate = true;
    }
    if (mesh3Ref.current) {
      mesh3Ref.current.count = count3;
      mesh3Ref.current.instanceMatrix.needsUpdate = true;
      if (mesh3Ref.current.instanceColor)
        mesh3Ref.current.instanceColor.needsUpdate = true;
    }
...
export default function App() {
  ...
  if (!started) {
    return (
      <div
        className="w-full h-screen relative overflow-hidden flex flex-col justify-center px-8 md:px-24"
      ...
      >
        ...
        <div className="z-10 max-w-2xl">
          <h1
            className="text-white text-6xl md:text-8xl font-semibold leading-[0.9] mb-6"
          ...
          <p className="text-white text-lg md:text-xl font-light mb-12 max-w-lg leading-relaxed">
            An AI-powered screen that responds to movement, transforming MSG’s
            new rebranding into an immersive experience.
          </p>
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 text-white/50 font-mono text-xs pointer-events-none tracking-widest z-10 mix-blend-difference">
        MSG INTERACTIVE WALL // REAL-TIME 3D DEMO
      </div>
      ...
      {showSettings && (
        <div className="absolute top-16 right-4 w-72 bg-black/80 backdrop-blur-md border border-white/10 rounded-2xl p-6 text-white z-20 shadow-2xl max-h-[80vh] overflow-y-auto">
          <h2 className="text-sm font-semibold mb-6 uppercase tracking-wider text-white/80">
            Settings
          </h2>
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              <div className="flex justify-between text-xs text-white/60">
                ...
                <span>
                  {resolution}x{Math.floor(resolution * 0.75)}
                </span>
              </div>
              <input
                type="range"
